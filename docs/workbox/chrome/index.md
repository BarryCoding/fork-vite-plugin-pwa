...
- You want to use other Service Worker features (i.e. [Web Push](https://developer.mozilla.org/en-US/docs/Web/API/Push_API)).
...
You can use the generateSW mode within a node-based build script, using the most common [configuration options](#generateswoptions), like so:
...
generateSW({
  ...
  globDirectory: '...',
  globPatterns: ['...', '...'],
  maximumFileSizeToCacheInBytes: 123,
  navigateFallback: '...',
  runtimeCaching: [{
...
A full set of configuration options can be found in the [reference documentation](#generateswoptions).
...
- [types.ts](https://github.com/GoogleChrome/workbox/blob/v7/packages/workbox-build/src/types.ts)
...
export interface BasePartial {
   ...
   * that might have accidentally matched one of your patterns. (2M)
   * @default 2097152
   */
...
```ts
workbox-build.generateSW(config: GenerateSWOptions)
```
